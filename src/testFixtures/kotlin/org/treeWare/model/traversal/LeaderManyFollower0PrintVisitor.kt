...
class LeaderManyFollower0PrintVisitor(private val writer: Writer) :
    ...
    override fun visitMain(leaderMainList: List<MainModel<Unit>?>): TraversalAction {
        val rootNames = leaderMainList.map { main -> main?.meta?.let { getRootMeta(it) }?.let { getMetaName(it) } }
        print("Main", rootNames)
        return TraversalAction.CONTINUE
    }

    override fun visitRoot(leaderRootList: List<RootModel<Unit>?>): TraversalAction {
        val rootEntityNames = leaderRootList.map { root -> root?.meta?.let { getMetaName(it) } }
        print("Root", rootEntityNames)
        return TraversalAction.CONTINUE
    }

    override fun visitEntity(leaderEntityList: List<EntityModel<Unit>?>): TraversalAction {
    ...
    override fun visitSingleField(leaderFieldList: List<SingleFieldModel<Unit>?>): TraversalAction =
        printFieldNames("Single field", leaderFieldList)

    override fun visitListField(leaderFieldList: List<ListFieldModel<Unit>?>): TraversalAction =
        printFieldNames("List field", leaderFieldList)

    override fun visitSetField(leaderFieldList: List<SetFieldModel<Unit>?>): TraversalAction =
        printFieldNames("Set field", leaderFieldList)

    override fun visitPrimitive(leaderValueList: List<PrimitiveModel<Unit>?>): TraversalAction {
    ...
    override fun visitAlias(leaderValueList: List<AliasModel<Unit>?>): TraversalAction {
        print("Alias")
        return TraversalAction.CONTINUE
    }

    override fun visitPassword1way(leaderValueList: List<Password1wayModel<Unit>?>): TraversalAction {
        val values = leaderValueList.map { it?.let { "{${it.unhashed}, ${it.hashed}, ${it.hashVersion}}" } }
        print("Password1way", values)
        return TraversalAction.CONTINUE
    }

    override fun visitPassword2way(leaderValueList: List<Password2wayModel<Unit>?>): TraversalAction {
        val values = leaderValueList.map { it?.let { "{${it.unencrypted}, ${it.encrypted}, ${it.cipherVersion}}" } }
        print("Password2way", values)
        return TraversalAction.CONTINUE
    }

    override fun visitEnumeration(leaderValueList: List<EnumerationModel<Unit>?>): TraversalAction {
        val values = leaderValueList.map { it?.value }
        print("Enumeration", values)
        return TraversalAction.CONTINUE
    }

    override fun visitAssociation(leaderValueList: List<AssociationModel<Unit>?>): TraversalAction {
